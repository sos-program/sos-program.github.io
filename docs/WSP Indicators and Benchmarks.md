...
<br>
The WSP status toolkit is used within the structured processes of [WSP integrated status assessments](./Integrated Status Assessments) or [WSP rapid status assessments](./WSP Rapid Status). The WSP Toolkit are criteria that that are selected from by CU experts to support these structured WSP status processes. This is the foundation to ensure status assessments are science-determined (peer-reviewed, objective, repeatable), consistently applied between CUs and years, and enable comprehensive coverage of all 377 'current' CUs with applicable data in B.C. and the Yukon.
